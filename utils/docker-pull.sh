#!/bin/bash
# This for pre-downloading current images to avoid
# "toomanyrequests: You have reached your pull rate limit. You may increase the limit by authenticating and upgrading"
# error on Portainer when deploying the stack
...
## let's automate it, shall we?
#grep 'image: ' ../docker-compose*yml|sed 's/.*image: //'|sort| uniq | sed "s/..OPENCTI_VERSION./`grep OPENCTI_VERSION= ../.env|cut -d'=' -f2`/" | sed 's/..MISP_IMAGE-//'| sed 's/}//g' | while read image; do docker pull $image;done